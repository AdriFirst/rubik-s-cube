    ...
    for y = 0 to sides-1 do
      for z = 0 to sides-1 do
        if (x = 0 || x = sides-1) && (y = 0 || y = sides-1) && (z = 0 || z = sides-1) then
          ...
          (* vertices *)
          cube.(x).(y).(z) <- Vertices [|Some Red;Some Blue;Some Orange|]
        else if (x = 0 || x = sides-1) || (y = 0 || y = sides-1) || (z = 0 || z = sides-1) then
          (* faces *)
          cube.(x).(y).(z) <- Faces [|Some Green;Some Orange;Some Orange|]
        else
          (* center(s) *)
          cube.(x).(y).(z) <- Center [|Some Orange;Some Orange;Some Orange|]
      done;
    done;
  done;
  cube

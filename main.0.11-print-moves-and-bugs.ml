    ...
    for i = 4-1 downto 1 do
      ...
      if i-1 = 0 then
        begin
          ...
          let x1, y1, z1 = cubies.(0) in
          Printf.printf "old cubie : (%i, %i, %i) " x y z; print_option_array c.(x).(y).(z) ; print_newline() ; Printf.printf "new cubie : (%i, %i, %i) " x1 y1 z1 ; print_option_array temp_cubies ; print_newline();
        (* que ce soit une arete ou un coin, on veut intervertir la couleur (possiblement None si arete) en y et celle en z *)

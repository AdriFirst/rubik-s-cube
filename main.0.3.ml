...
let print_cube_pattern_x3 (c:cube) : unit =
  ...
  for i = 0 to 5 do
    ...
    let i_co1, i_co2, i_co3 = get_index_of_values comb.(i) in
    (* on a maintenant la coordonnée qui reste fixe, et les coordonnées qui varient *)
    for co2 = 0 to 3-1 do
      for co3 = 0 to 3-1 do
        let cubie = c.(Option.get comb.(i).(i_co1)).(co2).(co3) in

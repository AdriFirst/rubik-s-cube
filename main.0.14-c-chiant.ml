...
let rec nth_cos (co:int*int) (n:int) (sides:int): int*int =
  (* permet d'obtenir la position d'un cubies de coordonnées co après n rotation. *)
  (* à noter que l'on peut composer un couple de 4 cubies, les quatres positions obtenables par chacun des cubies du couple par la rotation R.
   * En effet, il y a quatres cas possibles, le cycle est de période 4 *)
  let x0, y0 = co in
  match n with
  | 0 -> x0, y0
  | _ -> nth_cos (-y0+1, x0 + 1) (n-1) (sides)
...
let move_r (c:cube) : unit =
  ...
  for couple_nb = 0 to sides do
    for i_elt_of_couple = 0 to 3 do
      let x0, y0, z0 = sides-1, 0, couple_nb in
      c.(x0).(y0).(z0) <- let yn, zn = nth_cos (y0, z0) (i_elt_of_couple) (sides) in
        print_int x0 ; print_int y0 ; print_int z0 ; print_newline() ;
        print_int x0 ; print_int yn ; print_int zn ; print_newline() ; print_newline();
        (*print_option_array c.(x0).(y0).(z0) ; print_newline() ; print_option_array copy_c.(x0).(yn).(zn) ; print_newline(); print_newline();*)
        copy_c.(x0).(yn).(zn)

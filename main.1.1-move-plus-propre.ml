...
let move_r' (c:cube) : unit =
  ...
  let pos_succ_edges = [|(2, 0, 0) ; (2, 0, 2) ; (2, 2, 2) ; (2, 2, 0) ; (2, 0, 0)|] (* on remet la 1ere comb à la fin pour ne pas avoir à gerer de cas particuliers dans notre boucle *)
  and pos_succ_verti = [|(2, 0, 1) ; (2, 1, 2) ; (2, 2, 1) ; (2, 1, 0) ; (2, 0, 1)|] in
  ...
  (* edges  *)
  for i = 0 to 3 do
    let x0, y0, z0 = pos_succ_edges.(i) (* coordonnées actuelles du cubie *)
    and x1, y1, z1 = pos_succ_edges.(i+1) in (* futures coordonnées du cobie *)
    c.(x0).(y0).(z0) <- copy_c.(x1).(y1).(z1) ;
    c.(x0).(y0).(z0) <- exchange_edge_colors c.(x0).(y0).(z0) 'x'
  done;
  (* vertices *)
  for i = 0 to 3 do
    let x0, y0, z0 = pos_succ_verti.(i) (* coordonnées actuelles du cubie *)
    and x1, y1, z1 = pos_succ_verti.(i+1) in (* futures coordonnées du cobie *)
    c.(x0).(y0).(z0) <- copy_c.(x1).(y1).(z1) ;
    c.(x0).(y0).(z0) <- exchange_vertices_colors c.(x0).(y0).(z0) 'x'
  done

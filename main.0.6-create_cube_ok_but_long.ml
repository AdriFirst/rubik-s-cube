...
let create_cube (sides : int) : cube =
  let cube = Array.make sides [||] in for i = 0 to sides-1 do cube.(i) <- Array.make_matrix sides sides (Center [|Some Orange;Some Orange;Some Orange|]) done;
  for x = 0 to sides-1 do
    for y = 0 to sides-1 do
      for z = 0 to sides-1 do
        (* EDGE *)
        if (x = 0 || x = sides-1) && (y = 0 || y = sides-1) && (z = 0 || z = sides-1) then
          cube.(x).(y).(z) <- Edges ([|if x = 0 then Some Red else Some Orange; if y = 0 then Some White else Some Yellow ; if z = 0 then Some Blue else Some Green|])

        (* VERTICES *)
        else if (x = 0 || x = sides-1) && (y = 0 || y = sides-1) then
          (* X and Y *)
          cube.(x).(y).(z) <- Vertices [|if x = 0 then Some Red else Some Orange ; if y = 0 then Some White else Some Yellow ; None|]
        else if (y = 0 || y = sides-1) && (z = 0 || z = sides-1) then
          (* Y and Z *)
          cube.(x).(y).(z) <- Vertices [|None ; if y = 0 then Some White else Some Yellow ; if z = 0 then Some Blue else Some Green|]
        else if  (x = 0 || x = sides-1) && (z = 0 || z = sides-1) then
          (* X and Z *)
          cube.(x).(y).(z) <- Vertices [|if x = 0 then Some Red else Some Orange ; None ; if z = 0 then Some Blue else Some Green|]

        (* FACES *)
        else if (x = 0 || x = sides-1) then
          cube.(x).(y).(z) <- Faces [|if x = 0 then Some Red else Some Orange ; None ; None|]
        else if (y = 0 || y = sides-1) then
          cube.(x).(y).(z) <- Faces [|None ; if x = 0 then Some White else Some Yellow ; None|]
        else if (z = 0 || z = sides-1) then
          cube.(x).(y).(z) <- Faces [|None ; None ; if z = 0 then Some Blue else Some Green|]

...
let move_r (c:cube) : unit =
  ...
  for cycles = 0 to sides-1 do
    ...
    let x, y, z = cubies.(0) in let x1, y1, z1 = cubies.(3) in c.(x).(y).(z) <- c.(x1).(y1).(z1);
    if Option.is_none c.(x).(y).(z).(0) || Option.is_none c.(x).(y).(z).(1) || Option.is_none c.(x).(y).(z).(2) then
      (* c'est une arete  *)
      (* on ne change pas la couleur en x, mais celle en y devient en z *)
      begin
        c.(x).(y).(z).(2) <- c.(x).(y).(z).(1);
        c.(x).(y).(z).(2) <- None;
      end
    else
      (* c'est un coin (les centres et faces ne sont pas permutés) *)
      let temp_color = c.(x).(y).(z).(0) in
      c.(x).(y).(z).(0) <- c.(x).(y).(z).(1); (* c_X <- c_Y *)
      c.(x).(y).(z).(1) <- c.(x).(y).(z).(2); (* c_Y <- c_Z *)
      c.(x).(y).(z).(2) <- temp_color; (* c_Z <- c_X *)
    let x, y, z = cubies.(3) in let x1, y1, z1 = cubies.(2) in c.(x).(y).(z) <- c.(x1).(y1).(z1);
    let x, y, z = cubies.(2) in let x1, y1, z1 = cubies.(1) in c.(x).(y).(z) <- c.(x1).(y1).(z1);
    let x, y, z = cubies.(1) in c.(x).(y).(z) <- temp_cubies
  done;

  (* ROTATION DES COULEURS DES CUBIES *)
